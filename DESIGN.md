...
2. Заменяет в каждом токене переменные с помощью [Substitutor](#substitutor) (сначала токенизировали, а затем делаем подстановку, такое поведение схоже с поведением zsh)
